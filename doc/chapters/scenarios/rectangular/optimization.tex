\subsection{Optimization for the rectangular scenario (with low density)}\label{subsec:rectoptimization}
...
identified as relevant in \secref{subsec:rect2kr}, namely the maximum number of
copies and the size of the hear window. The configuration used is named
``LowDensityMessages''.

As we can see from \figref{fig:rectmessagesff}, we get that the maximum number of
...
``LowDensityCollisions''. \figref{fig:rectcollisionsff} shows the results:
